...
auto col_i = tr.ReadColumn<Int>(1, [](auto v){return stoi(v);});

auto ixs_cols_d = vector<int>{1, 2};
auto cols_d = tr.ReadColumns<Real>(ixs_cols_d, [](auto v){return stod(v);});

VectorXi vec_i = Map<VectorXi>(&col_i[0], col_i.size());
MatrixXd mat_d = Map<MatrixXd>(&cols_d[0], tr.n_rows(), ixs_cols_d.size());

auto columns = make_tuple(vec_i, mat_d);
auto df = DataFrame(columns);

TEST(DataFrame, Apply_Fn) {
  auto sum_df = df.Apply<Real>([](auto c){return c.sum();});
...
TEST(DataFrame, Apply_Fn_Int_Int) {
  auto sum_1 = df.Apply<Real>([](auto c){return c.sum();}, 0, 0);
  auto sum_2 = df.Apply<Real>([](auto c){return c.sum();}, 0, 1);
  auto sum_3 = df.Apply<Real>([](auto c){return c.sum();}, 0, 5);
  auto sum_4 = df.Apply<Real>([](auto c){return c.sum();}, 3, 6);
...
TEST(DataFrame, Roll) {
  auto roll_1 = df.Roll<Real>([](auto w){return w.mean();}, 2);
  auto roll_2 = df.Roll<Real>([](auto w){return w.mean();}, 3);
